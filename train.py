...
from config import (model_config, data_generator_config, load_from_file,
                    n_epochs, model_name, test_seq_path)
...
val_X = np.array([x for x in next(test_data_generator) for _ in range(20)])
...
lr_cb = ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=2, verbose=1,
                          min_lr=0.0001)
